...
export function MindSpaceApp() {
  ...
  const { boards, activeBoardId, setActiveBoardId, createBoard, updateBoardDocument, syncStatus, syncError } = usePersistedBoards();
  ...
  return <>
    <AppShell sidebar={<WorkspaceSidebar boards={boards} activeBoardId={activeBoardId} onCreateBoard={createBoard} onSelectBoard={selectBoard} />} topbar={<WorkspaceTopbar engine={engine} boardName={activeBoard?.name ?? "MindSpace"} syncStatus={syncStatus} syncError={syncError} onOpenAi={() => setAiOpen(true)} />} board={activeBoard ? <BoardCanvas key={activeBoardId} document={activeBoard.document} onDocumentChange={(document) => updateBoardDocument(activeBoard.id, document)} onEngineReady={handleEngineReady} /> : <div className="grid h-full place-items-center text-sm text-muted-foreground">{t("syncConnecting")}…</div>} rightPanel={<AiPanel />} />
