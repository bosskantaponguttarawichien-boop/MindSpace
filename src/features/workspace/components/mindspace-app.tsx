...
export function MindSpaceApp() {
  ...
  return <>
    <AppShell sidebar={<WorkspaceSidebar boards={boards} activeBoardId={activeBoardId} nextBoardName={nextBoardName} onCreateBoard={createBoard} onRenameBoard={renameBoard} onDeleteBoard={deleteBoard} onSelectBoard={selectBoard} />} topbar={<WorkspaceTopbar engine={engine} boardName={activeBoard?.name ?? "MindSpace"} boards={boards} activeBoardId={activeBoardId} nextBoardName={nextBoardName} syncStatus={syncStatus} syncError={syncError} onCreateBoard={createBoard} onSelectBoard={selectBoard} onCopySyncLink={copySyncLink} onExportPdf={requestExport} onOpenAi={() => setAiOpen(true)} />} board={activeBoard ? <BoardCanvas key={activeBoardId} document={activeBoard.document} onDocumentChange={handleDocumentChange} onUploadImage={uploadImage} onDeleteImages={deleteImages} onOpenPdf={setPdf} onEngineReady={handleEngineReady} /> : <div className="grid h-full place-items-center text-sm text-muted-foreground">{t("syncConnecting")}…</div>} rightPanel={<AiPanel />} />
